...
/* ════════════════════════════════════════════════════
   ★ MODERN API POLYFILLS (minimal & safe)
   ════════════════════════════════════════════════════ */
...
const AEGIS_CONFIG = Object.freeze({
    ...
    SUGGESTION_CACHE_MAX: 50,         // ★ NEW
    PREFETCH_URL_CACHE_MAX: 250,      // ★ NEW
    MEMO_SORT_CACHE_MAX: 30,          // ★ NEW
    MIN_YEAR: 2023,
    SESSION_SEED: (() => {
        try {
            if (crypto?.randomUUID) {
                return parseInt(crypto.randomUUID().replace(/-/g, '').slice(0, 8), 16) % 99999;
            }
        } catch {}
        return Math.floor(Math.random() * 99999);
    })(),
    WRITE_FRAME_BUDGET: 8,
    DB_NAME: 'AEGIS_DB_V52_ULTRA',
    SENTINEL_ID: 'loadMoreSentinel',
    PRECONNECT_HOSTS: ['https://graphql.anilist.co', 'https://s4.anilist.co', 'https://s3.anilist.co']
});

/* ════════════════════════════════════════════════════
   ★ INJECT PRECONNECT HINTS (one-time, early)
   ════════════════════════════════════════════════════ */
(() => {
    if (document.getElementById('aegis-preconnect')) return;
    const frag = document.createDocumentFragment();
    for (const host of AEGIS_CONFIG.PRECONNECT_HOSTS) {
...
/* ════════════════════════════════════════════════════
   ★ LRU CACHE (bound memory, no leaks)
   ════════════════════════════════════════════════════ */
class LRU {
...
/* ════════════════════════════════════════════════════
   ★ SHARED SCROLL MANAGER (single rAF-throttled listener)
   ════════════════════════════════════════════════════ */
class ScrollManager {
    static #instance = null;
    static get() {
        if (!ScrollManager.#instance) ScrollManager.#instance = new ScrollManager();
    ...
    isScrolling() { return this.scrolling; }
    #tick() {
...
/* ════════════════════════════════════════════════════
   ★ SHARED IMAGE OBSERVER (one per page)
   ════════════════════════════════════════════════════ */
class SharedImageObserver {
    static #instance = null;
    static get() {
        if (!SharedImageObserver.#instance) {
            SharedImageObserver.#instance = new SharedImageObserver();
        }
        return SharedImageObserver.#instance;
    }
    constructor() {
        this.obs = null;
            ...
            this.obs = new IntersectionObserver((entries) => {
                for (const e of entries) {
                    if (e.isIntersecting) {
                        ...
                        if (cb) { cb(); this.callbacks.delete(e.target); }
    ...
    observe(el, cb) {
        if (!el) return;
        if (!this.obs) { cb(); return; }
        this.pending.add(el);
        this.callbacks.set(el, cb);
        this.obs.observe(el);
    }
    unobserve(el) {
        if (!this.obs || !el) return;
    ...
    destroy() {
        ...
        this.pending.clear();
        SharedImageObserver.#instance = null;
    }
}

/* ════════════════════════════════════════════════════
   ★ SMART SORT WORKER (Blob)
   ════════════════════════════════════════════════════ */
...
function smartSort(list, profile, now) {
    ...
    // ★ Stable sort with fallback
    const sorted = (typeof Array.prototype.toSorted === 'function')
        ? scored.toSorted((x, y) => y.s - x.s)
        : scored.slice().sort((x, y) => y.s - x.s);
...
/* ════════════════════════════════════════════════════
   ★ WORKER POOL (singleton, reusable)
   ════════════════════════════════════════════════════ */
class WorkerPool {
    static #instance = null;
    static get() {
        if (!WorkerPool.#instance) WorkerPool.#instance = new WorkerPool();
        return WorkerPool.#instance;
    }
    static hasInstance() { return !!WorkerPool.#instance; }
    constructor() {
        this.workers = [];
        this.idle = [];
        this.queue = [];
    ...
    #init() {
        ...
        const n = Math.min(2, Math.max(1, cores - 1));
    ...
    #spawn() {
        try {
            ...
            w.onmessage = (e) => this.#onMessage(e, w);
            w.onerror = (err) => {
                console.warn('[WorkerPool] worker error:', err);
            ...
            console.warn('[WorkerPool] spawn failed:', e);
        }
    }
    #onMessage(e, w) {
        const { id, result, ok } = e.data;
    ...
    exec(data) {
        if (!this.workers.length) return Promise.resolve(null);
    ...
    #drain() {
        while (this.queue.length && this.idle.length) {
            ...
            try {
                w.postMessage({ ...job.data, id: job.id });
            } catch {
                this.resolvers.get(job.id)?.resolve(null);
    ...
    destroy() {
        ...
        this.idle = [];
        this.queue = [];
        this.resolvers.forEach(r => r.resolve(null));
...
/* ════════════════════════════════════════════════════
   ★ DB (with batching & auto-cleanup)
   ════════════════════════════════════════════════════ */
class AegisDB {
    constructor(name = AEGIS_CONFIG.DB_NAME, version = 3) {
        this.name = name;
    ...
    async get(s, k) {
        if (this.useFallback) {
            try { return JSON.parse(localStorage.getItem(`${this.name}:${s}:${k}`)); }
            catch { return null; }
        }
    ...
    _enqueueBatch() {
        ...
        this._batchTimer = setTimeout(() => { this._flushBatch().catch(() => {}); }, 50);
    }
    async _flushBatch() {
        ...
        try {
            ...
            return promise;
        } catch {} finally { this._batchRunning = false; }
    }
    async set(s, k, v) {
        if (this.useFallback) {
            try { localStorage.setItem(`${this.name}:${s}:${k}`, JSON.stringify({ k, v })); } catch {}
            return;
        }
    ...
    async del(s, k) {
        if (this.useFallback) { localStorage.removeItem(`${this.name}:${s}:${k}`); return; }
    ...
    async cleanupOldVersions() {
        ...
        try {
            ...
            for (const dbInfo of dbs) {
                const m = String(dbInfo.name || '').match(/_V(\d+)/);
                if (m && dbInfo.name?.startsWith('AEGIS_DB_')) {
                    const dbVersion = parseInt(m[1], 10);
...
class AegisEngine {
    ...
    #sugCache = new LRU(AEGIS_CONFIG.SUGGESTION_CACHE_MAX);
    #prefetchUrls = new LRU(AEGIS_CONFIG.PREFETCH_URL_CACHE_MAX);
    #memoSort = new LRU(AEGIS_CONFIG.MEMO_SORT_CACHE_MAX);
    #writeQueue = [];
    #writeRAF = 0;
    #resizeRAF = 0;
    #tSearch = 0;
    ...
    constructor(rootElement) {
        ...
        this.db = new AegisDB();
        // ★ Shared singletons
        this.#imgObs = SharedImageObserver.get();
        this.#scrollMgr = ScrollManager.get();
    }

    /* ── Smart Sort via Worker Pool ──────────────── */
    async #sortInWorker(list) {
        if (!list?.length) return list;
        if (!WorkerPool.hasInstance()) return this.#smartSortFallback(list);

        const sorted = await WorkerPool.get().exec({
            list: list.map(a => ({
                id: a.id,
    ...
    #safeParse(i, fb) {
        ...
        try { return JSON.parse(i); } catch { return fb; }
    }
    ...
    #invalidateMemo() { this.#memoSort.clear(); }
    ...
    async #purgeCache() {
        try {
            ...
            if (valid.length > 20) {
                ...
                const rm = Math.ceil(sorted.length / 2);
                for (let i = 0; i < rm; i++) await this.db.del('c', sorted[i].k);
            }
        } catch {}
    }
    ...
    #cacheDom() {
        ...
        if (this.#dom.sliderTrk) {
            this.#dom.sliderTrk.style.cssText += ';direction:ltr;will-change:transform;';
        }
    }

    #injectStyles() {
        if (document.getElementById('aegis52-css')) return;
        const s = document.createElement('style');
        s.id = 'aegis52-css';
        s.textContent = `
.anime-card,.slide,.suggestion-item{touch-action:pan-y}
.anime-card{
    position:relative;border-radius:16px;overflow:hidden;aspect-ratio:2/3;cursor:pointer;
...
.anime-card.visible{animation:aegis-up .28s ease both}
.anime-card:hover{transform:translateY(-5px) scale(1.02);box-shadow:0 14px 38px rgba(0,0,0,.35),0 0 0 1px rgba(100,180,255,.15);z-index:2}
...
.anime-card .card-glow{position:absolute;inset:0;background:radial-gradient(120px circle at var(--mx,50%) var(--my,50%),rgba(100,180,255,.14),transparent 60%);opacity:0;transition:opacity .2s;pointer-events:none;z-index:1}
.anime-card:hover .card-glow{opacity:1}
body.is-scrolling .anime-card .card-glow{display:none}
.anime-card .card-info{position:absolute;bottom:0;left:0;right:0;padding:36px 10px 10px;background:linear-gradient(0deg,rgba(6,14,32,.98) 0%,rgba(6,14,32,.75) 55%,transparent 100%);z-index:2;pointer-events:none;transition:backdrop-filter .2s ease,-webkit-backdrop-filter .2s ease}
.anime-card:hover .card-info{backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
...
@keyframes aegis-up{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
...
@media(prefers-reduced-motion:reduce){
    .anime-card.visible,.anime-card .card-img,.skeleton-card::after,.anime-card .live-dot{animation:none!important;transition:none!important}
}
`;
    ...
    #observeImg(img) {
        if (!img) return;
        if (this.#imgObs) this.#imgObs.observe(img, () => this.#lazyImg(img));
        else this.#lazyImg(img);
    }
    ...
    #warmImages(list, hero = false) {
        const urls = (list ?? [])
            ...
            .filter(Boolean)
            .slice(0, hero ? (this.#touch ? 4 : 8) : (this.#touch ? 2 : AEGIS_CONFIG.PREFETCH_MAX_IMAGES));

        for (const u of urls) {
            if (this.#prefetchUrls.has(u)) continue;
            this.#prefetchUrls.set(u, 1);
            const img = new Image();
            img.decoding = 'async';
    ...
    async #gql(query, vars, ctl, timeout = AEGIS_CONFIG.API_TIMEOUT) {
        // ★ Modern: combine user abort + timeout in one signal
        const combined = AbortSignal.any
            ? AbortSignal.any([ctl.signal, AbortSignal.timeout(timeout)])
            : (() => {
                const ac = new AbortController();
                const onAbort = () => ac.abort(ctl.signal.reason);
                ctl.signal.addEventListener('abort', onAbort, { once: true });
                const tid = setTimeout(() => ac.abort(new DOMException('Timeout', 'TimeoutError')), timeout);
        ...
        try {
            ...
            return { ok: false, err, timedOut: err?.name === 'TimeoutError' };
        }
    }
    ...
    #renderSlider(list) {
        ...
        const html = list.map((a, i) => {
            ...
            return `<div class="slide" data-url="/anime/${b64(a.id)}/${slug}" data-id="${a.id}" data-title="${ttl}" data-genres="${esc(encodeURIComponent(JSON.stringify(a.genres || [])))}" data-format="${esc(a.format || '')}"><img class="slide-media" src="${esc(img)}" alt="${ttl}" loading="${i === 0 ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${i === 0 ? 'high' : 'low'}" onerror="this.src='https://via.placeholder.com/1200x600/0E2044/BDE8F5?text=—'"><div class="slide-overlay"></div><div class="slide-content"><h2 class="slide-title">${ttl}</h2><div class="slide-meta"><span class="slide-badge">★ ${sc}</span>${a.seasonYear ? `<span class="slide-meta-chip">${esc(a.seasonYear)}</span>` : ''}${a.format ? `<span class="slide-meta-chip">${esc(a.format)}</span>` : ''}</div></div></div>`;
        }).join('');
        ...
        this.#write(() => {
            ...
            // ★ replaceChildren أسرع من innerHTML
            sliderTrk.replaceChildren();
            sliderTrk.insertAdjacentHTML('beforeend', html);
            sliderTrk.style.cssText += ';display:flex;transform:translate3d(0,0,0);';
            sliderDots.replaceChildren();
            sliderDots.insertAdjacentHTML('beforeend', dots);
    ...
    async #fetchSlider() {
        ...
        const cached = await this.#cacheGet('slider_v52');
        ...
        await this.#cacheSet('slider_v52', list, true);
        if (this.#dead || seq !== this.#seq.slider) return;
        this.#sliderData = list;
    ...
    #renderSkeleton() {
        const grid = this.#dom.grid;
        if (!grid) return;
        // ★ DocumentFragment أسرع من innerHTML
        const frag = document.createDocumentFragment();
        for (let i = 0; i < 12; i++) {
        ...
        this.#write(() => grid.replaceChildren(frag));
    }
    ...
    #renderError(msg) {
        ...
        if (!grid) return;
        this.#write(() => {
            grid.innerHTML = `<div class="soft-fail">${esc(msg)}<div><button type="button" id="retryBtn">إعادة المحاولة</button></div></div>`;
    ...
    #renderEmpty() {
        ...
        if (!grid) return;
        this.#write(() => {
            grid.innerHTML = '<div class="no-results">لا توجد نتائج تطابق بحثك أو الفلاتر المحددة</div>';
        });
    }
    ...
    #renderCards(list) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            // ★ أسرع animationDelay مع حد أعلى
            card.style.animationDelay = `${Math.min(i * 15, 120)}ms`;
    ...
    async #fetchGrid() {
        ...
        if (this.#dead || reqId !== this.#seq.grid) { this.#state.loading = false; return; }
        if (cached) {
        ...
        if (this.#dead || reqId !== this.#seq.grid) { this.#state.loading = false; this.#hideSpin(); return; }
        if (!ok) {
            if (!timedOut && ctl.signal.aborted) { this.#state.loading = false; this.#hideSpin(); return; }
            if (this.#state.page === 1) {
                this.#renderError('حدثت مشكلة في تحميل البيانات');
                this.#vib([20, 40, 20]);
            }
            this.#state.loading = false; this.#hideSpin(); return;
        }

        const page = data?.data?.Page;
        if (!page?.media) { this.#state.loading = false; this.#hideSpin(); return; }
        if (this.#state.page === 1) this.#dom.grid.innerHTML = '';

        let list = page.media;
        const shouldSort = !this.#state.query && !this.#active.genre && !this.#active.format && !this.#active.year && !this.#active.status;
        if (shouldSort) {
            const memo = this.#memoSort.get(ck);
            if (memo) list = memo;
            else {
        ...
        if (!list.length && this.#state.page === 1) this.#renderEmpty();
        else {
    ...
    async #prefetch() {
        ...
        try {
            ...
            const { ok, data } = await this.#gql(this.#gridQuery(), vars, ctl);
    ...
    async #fetchSug(q) {
        ...
        if (n.length < 2) {
            this.#write(() => {
                if (this.#dom.sugBox) {
                    this.#dom.sugBox.replaceChildren();
        ...
        const cached = this.#sugCache.get(n);
        if (cached) { this.#renderSug(cached, n); return; }
        ...
        const { ok, data } = await this.#gql(gql, { s: q }, ctl);
        if (this.#dead || reqId !== this.#seq.sug) return;
        if (!ok) return;

        const list = data?.data?.Page?.media ?? [];
        this.#sugCache.set(n, list);
    ...
    #renderSug(list, q = '') {
        ...
        const ranked = (typeof Array.prototype.toSorted === 'function')
            ? list.toSorted(rank)
            : list.slice().sort(rank);
        ...
        const html = ranked.map(a => {
            ...
            return `<button class="suggestion-item" type="button" data-url="/anime/${b64(a.id)}/${slug}" data-id="${a.id}" data-title="${esc(title)}"><img src="${esc(a.coverImage?.medium || '')}" class="suggestion-img" alt="${esc(title)}" loading="lazy" decoding="async"><div class="suggestion-info"><div class="suggestion-title">${highlight(title)}</div><div class="suggestion-meta">${a.seasonYear ? `<span>${esc(a.seasonYear)}</span>` : ''}<span>${esc(a.format || 'TV')}</span><span>★ ${a.averageScore ? (a.averageScore / 10).toFixed(1) : '—'}</span></div></div></button>`;
        }).join('');
    ...
    #bindTap(container, selector, fn, opts = {}) {
        ...
        const hUp = e => {
            ...
            if (ok) {
                ...
                if (title) this.#write(() => { document.title = String(title).trim(); });
                fn(el, e);
            }
        };
    ...
    #bindUI() {
            ...
            const hKey = e => {
                if (e.key === 'ArrowDown') { e.preventDefault(); this.#moveSug(1); }
                    ...
                    if (this.#sugIdx >= 0 && items[this.#sugIdx]) {
                        e.preventDefault();
                        items[this.#sugIdx].dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true, pointerType: 'mouse', button: 0 }));
                    }
        ...
        this.#bindTap(grid, '.anime-card', card => {
            ...
            if (!url) return;
            this.#vib(10);
            this.#track(this.#safeParse(decodeURIComponent(card.dataset.genres || '[]'), []), card.dataset.format || '');
        ...
        if (grid) {
            ...
            grid.addEventListener('keydown', hCardKey, { passive: false });
            this._listeners.push({ el: grid, ev: 'keydown', fn: hCardKey });
        }
        ...
        if (filterBtn && filterPop) {
            const hToggle = e => {
                ...
                if (open) { this.#pending = { ...this.#active }; this.#syncPopover(); }
                this.#vib(10);
            };
        ...
        // ★ Scroll موحد مع shared manager
        this.#unsubScroll = this.#scrollMgr.subscribe(() => {
            // any per-engine scroll logic can go here
        });
    ...
    destroy() {
        ...
        if (this.#unsubScroll) this.#unsubScroll();
        ...
        // ★ LRU caches have built-in clear()
        this.#sugCache.clear();
